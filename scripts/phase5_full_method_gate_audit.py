...
def build_claim_gate(summaries: dict[str, dict[str, Any]]) -> pd.DataFrame:
    ...
    mv14_anchor_summary = (
        f"MV14 run is {mv14_status}, core effective R {mv14_result.get('core_effective_draws')}, "
        ...
        f"{';'.join(mv14_result.get('top_threshold_dif_items') or []) or 'none'}, and best AIC/BIC "
        f"{mv14_result.get('best_aic_model')}/{mv14_result.get('best_bic_model')}"
    )
    ...
    rows = [
        ...
        },
        {
            ...
            "allowed_scope": "Use MV10/MV11/MV13/MV14 as label-only PHQ partial-invariance evidence with measured uncertainty, and MV12 plus its aggregate tradeoff analysis as bounded two-stage prediction diagnostics; do not present them as multimodal method success or external scale transfer.",
            ...
            "required_next_evidence": "Predeclare MV15/MV16 if moving beyond label-measurement evidence toward latent identity or scale-linking claims.",
...
def build_next_actions(summaries: dict[str, dict[str, Any]]) -> pd.DataFrame:
    ...
    if mv14_result.get("status"):
        shared_feature_action = {
            ...
            "why_now": f"MV14 is {mv14_result.get('status')}: anchors are stable under bootstrap and C02/C06 threshold-DIF remains localized, but full method is still blocked because this is label-only measurement uncertainty.",
            "success_gate": "MV15 specifies how to compare dataset identity conditioned on labels, theta, and legitimate covariates with aggregate-only outputs and local-only theta or residual tables.",
