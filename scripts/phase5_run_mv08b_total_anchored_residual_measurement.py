#!/usr/bin/env python3
"""Run P5_MV08b total-anchored residual measurement pilot.
...
- total-anchored sparse residual item heads.
...
DEFAULT_OUT_DIR = ROOT / "analysis" / "phase5_minimal_validation" / "p5_mv08b_total_anchored_residual_measurement"
...
RUN_ID = "P5_MV08b_total_anchored_residual_measurement"
SEEDS = [0, 1, 2, 3, 4]
...
LOCAL_PREDICTIONS_NAME = "p5_mv08b_local_row_predictions.csv"
CURRENT_MV08_M2_PREDICTION_IDENTITY_BA = 0.9002020202020201
...
TRACKED_FILES = {
    ...
    "report.md",
    "residual_model_audit.csv",
    "run_summary.json",
    "threshold_sparsity_summary.csv",
}
...
def item_mean_predictions(train: pd.DataFrame, eval_frame: pd.DataFrame, item_specs: list[ItemSpec], seed: int, fold: str, protocol: str) -> pd.DataFrame:
    ...
    for _, subject in eval_frame.iterrows():
        for item_id, y_true in subject["item_values"].items():
            ...
            rows.append(prediction_row(subject, spec, y_true, pred, seed, fold, protocol, "B0_train_mean_items"))
    return pd.DataFrame(rows)
    ...
    for (dataset, scale), eval_group in eval_frame.groupby(["dataset", "scale"], sort=False):
        ...
        for row_idx, (_, subject) in enumerate(eval_group.iterrows()):
            for item_id, y_true in subject["item_values"].items():
                ...
                rows.append(prediction_row(subject, spec, y_true, pred, seed, fold, protocol, "B1_total_score_floor"))
    return pd.DataFrame(rows)
...
def fixed_map_predictions(train: pd.DataFrame, eval_frame: pd.DataFrame, feature_cols: list[str], item_specs: list[ItemSpec], seed: int, fold: str, protocol: str) -> tuple[pd.DataFrame, dict[str, Any], pd.DataFrame]:
    ...
    for _, subject in eval_frame.iterrows():
        ...
        for item_id, y_true in subject["item_values"].items():
            ...
            rows.append(prediction_row(subject, spec, y_true, pred, seed, fold, protocol, "B2_fixed_construct_map"))
...
def total_anchor_item_frame(train: pd.DataFrame, target_frame: pd.DataFrame, feature_cols: list[str], item_specs: list[ItemSpec], seed: int) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    spec_by_id = {spec.item_id: spec for spec in item_specs}
    for (dataset, scale), target_group in target_frame.groupby(["dataset", "scale"], sort=False):
        train_group = train[(train["dataset"] == dataset) & (train["scale"] == scale)].copy()
        if train_group.empty:
            continue
        x_train = train_group[feature_cols].to_numpy(dtype=float)
        ...
        model = ridge_pipeline(alpha)
        model.fit(x_train, y_train)
        total_pred = np.asarray(model.predict(target_group[feature_cols].to_numpy(dtype=float)), dtype=float).reshape(-1)
        train_obs = item_observations(train_group, item_specs)
        item_means = train_obs.groupby("item_id")["y_true"].mean().to_dict()
        denom = float(sum(item_means.values()))
        item_ids = [spec.item_id for spec in item_specs if spec.dataset == dataset and spec.scale == scale]
        fallback_prop = 1.0 / len(item_ids) if item_ids else 0.0
        scale_max = float(sum(spec_by_id[item_id].item_max for item_id in item_ids)) if item_ids else 1.0
        for row_idx, (_, subject) in enumerate(target_group.iterrows()):
            raw_total_pred = float(total_pred[row_idx])
            for item_id, y_true in subject["item_values"].items():
                spec = spec_by_id[item_id]
                prop = (float(item_means.get(item_id, 0.0)) / denom) if denom > 0 else fallback_prop
                base_pred = float(np.clip(raw_total_pred * prop, 0.0, spec.item_max))
                rows.append(
                    {
                        "subject_key": subject["subject_key"],
                        "subject_id": subject["subject_id"],
                        ...
                        "total_anchor_pred": raw_total_pred,
                        "total_anchor_norm": float(np.clip(raw_total_pred / scale_max, 0.0, 1.0)),
                        "base_pred": base_pred,
                        "base_pred_norm": base_pred / float(spec.item_max),
                        "y_true": float(y_true),
                    }
                )
    return pd.DataFrame(rows)
...
class ResidualHead:
    ...
    def predict(self, x: np.ndarray) -> np.ndarray:
        if self.kind == "constant" or self.model is None:
            return np.repeat(float(self.constant_residual), x.shape[0])
        return np.asarray(self.model.predict(x), dtype=float).reshape(-1)


def residual_feature_frame(anchor: pd.DataFrame, subjects: pd.DataFrame, feature_cols: list[str]) -> pd.DataFrame:
    subject_features = subjects[["subject_key", *feature_cols]].copy()
    frame = anchor.merge(subject_features, on="subject_key", how="left", validate="many_to_one")
    frame["residual_target"] = frame["y_true"].astype(float) - frame["base_pred"].astype(float)
    return frame


def residual_feature_columns(feature_cols: list[str]) -> list[str]:
    return [*feature_cols, "base_pred_norm", "total_anchor_norm"]


def fit_residual_heads(train: pd.DataFrame, train_anchor: pd.DataFrame, feature_cols: list[str], item_specs: list[ItemSpec], seed: int) -> tuple[dict[str, ResidualHead], pd.DataFrame]:
    spec_by_id = {spec.item_id: spec for spec in item_specs}
    train_features = residual_feature_frame(train_anchor, train, feature_cols)
    x_cols = residual_feature_columns(feature_cols)
    heads: dict[str, ResidualHead] = {}
    audit_rows: list[dict[str, Any]] = []
    for head_group, group in train_features.groupby("head_group", sort=True):
        specs = [spec_by_id[item_id] for item_id in sorted(group["item_id"].unique(), key=natural_key)]
        x = group[x_cols].to_numpy(dtype=float)
        y = group["residual_target"].to_numpy(dtype=float)
        finite = np.isfinite(y)
        if int(finite.sum()) < 8 or len(np.unique(np.round(y[finite], 6))) < 2:
            kind = "constant"
            selected_alpha = None
            model = None
            constant = float(np.nanmean(y[finite])) if int(finite.sum()) else 0.0
        else:
            selected_alpha = choose_alpha(x[finite], y[finite], seed)
        ...
        audit_rows.append(
            {
                ...
                "residual_model_kind": kind,
                "selected_alpha": selected_alpha,
                "residual_target_mean": safe_float(np.mean(y)) if len(y) else None,
                "residual_target_mae_if_zero": safe_float(np.mean(np.abs(y))) if len(y) else None,
                "zero_sum_centering_applied": True,
                "learned_parameter_exported": False,
            }
        )
    return heads, pd.DataFrame(audit_rows)


def threshold_sparsity_audit(train_anchor: pd.DataFrame, seed: int, fold: str, protocol: str) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    for (dif_policy, head_group), group in train_anchor.groupby(["dif_policy", "head_group"], sort=True):
        item_category_rows: list[dict[str, Any]] = []
        for item_id, item_group in group.groupby("item_id", sort=True):
            item_max = int(item_group["item_max"].max())
            counts = item_group["y_true"].round().astype(int).value_counts().to_dict()
            observed = [int(counts.get(level, 0)) for level in range(item_max + 1)]
            item_category_rows.append(
                {
                    "item_id": item_id,
                    "zero_category_count": int(sum(1 for count in observed if count == 0)),
                    "min_nonzero_category_count": int(min([count for count in observed if count > 0], default=0)),
                    "rare_category_count": int(sum(1 for count in observed if 0 < count < 5)),
                    "category_count": int(item_max + 1),
                }
            )
        if not item_category_rows:
            continue
        item_frame = pd.DataFrame(item_category_rows)
        rows.append(
            {
                "seed": int(seed),
                "fold": fold,
                ...
                "dif_policy": dif_policy,
                "head_group": head_group,
                "item_count": int(len(item_frame)),
                "category_count_total": int(item_frame["category_count"].sum()),
                "zero_category_count_total": int(item_frame["zero_category_count"].sum()),
                "rare_category_count_total": int(item_frame["rare_category_count"].sum()),
                "min_nonzero_category_count_min": int(item_frame["min_nonzero_category_count"].min()),
                "threshold_models_learned": 0,
                "constant_threshold_fraction": 0.0,
                "threshold_policy_status": "thresholds_not_independently_fit_residual_continuous",
            }
        )
    return pd.DataFrame(rows)


def total_anchored_residual_predictions(train: pd.DataFrame, eval_frame: pd.DataFrame, feature_cols: list[str], item_specs: list[ItemSpec], seed: int, fold: str, protocol: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    train_anchor = total_anchor_item_frame(train, train, feature_cols, item_specs, seed)
    eval_anchor = total_anchor_item_frame(train, eval_frame, feature_cols, item_specs, seed)
    heads, residual_audit = fit_residual_heads(train, train_anchor, feature_cols, item_specs, seed)
    threshold_audit = threshold_sparsity_audit(train_anchor, seed, fold, protocol)

    eval_features = residual_feature_frame(eval_anchor, eval_frame, feature_cols)
    x_cols = residual_feature_columns(feature_cols)
    raw_residuals: list[float] = []
    for _, row in eval_features.iterrows():
        head = heads.get(str(row["head_group"]))
        if head is None:
            raw_residuals.append(0.0)
            continue
        raw_residuals.append(float(head.predict(row[x_cols].to_numpy(dtype=float).reshape(1, -1))[0]))
    eval_features["raw_residual_pred"] = raw_residuals
    eval_features["centered_residual_pred"] = eval_features["raw_residual_pred"] - eval_features.groupby("subject_key")["raw_residual_pred"].transform("mean")
    eval_features["y_pred"] = np.clip(
        eval_features["base_pred"].astype(float) + eval_features["centered_residual_pred"].astype(float),
        0.0,
        eval_features["item_max"].astype(float),
    )
    eval_features["residual_sum_after_centering"] = eval_features.groupby("subject_key")["centered_residual_pred"].transform("sum")

    spec_by_id = {spec.item_id: spec for spec in item_specs}
    rows: list[dict[str, Any]] = []
    for _, row in eval_features.iterrows():
        spec = spec_by_id[str(row["item_id"])]
        out = prediction_row(row, spec, float(row["y_true"]), float(row["y_pred"]), seed, fold, protocol, "M2b_total_anchored_residual_measurement")
        out["base_pred"] = float(row["base_pred"])
        out["raw_residual_pred"] = float(row["raw_residual_pred"])
        out["centered_residual_pred"] = float(row["centered_residual_pred"])
        out["residual_sum_after_centering"] = float(row["residual_sum_after_centering"])
        rows.append(out)

    residual_audit.insert(0, "protocol", protocol)
    residual_audit.insert(0, "fold", fold)
    residual_audit.insert(0, "seed", seed)
    return pd.DataFrame(rows), residual_audit, threshold_audit
...
def prediction_row(subject: pd.Series, spec: ItemSpec, y_true: float, y_pred: float, seed: int, fold: str, protocol: str, model: str) -> dict[str, Any]:
    ...
    total_true = safe_float(subject.get("total_score"))
    if total_true is None:
        total_true = safe_float(subject.get("scale_total_true"))
    return {
        "run_id": RUN_ID,
        "protocol": protocol,
        "model": model,
        ...
        "dif_policy": spec.dif_policy,
        "item_max": int(spec.item_max),
        "scale_total_true": float(total_true if total_true is not None else 0.0),
        "y_true": float(y_true),
...
def run_experiments(subjects: pd.DataFrame, feature_cols: list[str], item_specs: list[ItemSpec], split_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for seed, protocol, fold, train, eval_frame in protocol_splits(subjects, split_path):
        ...
        residual_pred, residual_audit, threshold_audit = total_anchored_residual_predictions(
            train,
            eval_frame,
            feature_cols,
            item_specs,
        ...
        residual_rows.append(residual_audit)
        threshold_rows.append(threshold_audit)
    return (
...
def comparison_summary(metric_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    for (protocol, dataset, scale), group in macro.groupby(["protocol", "dataset_slice", "scale"], sort=False):
        ...
        train_mean = values.get("B0_train_mean_items")
        total = values.get("B1_total_score_floor")
        fixed = values.get("B2_fixed_construct_map")
...
def verdict_from_outputs(comparison: pd.DataFrame, identity_summary: pd.DataFrame) -> dict[str, Any]:
    pooled = comparison[
        ...
        & (comparison["model"] == "M2b_total_anchored_residual_measurement")
    ].copy()
    ...
    for _, row in active.iterrows():
        total_delta = safe_float(row.get("delta_vs_total_score_floor"))
        fixed_delta = safe_float(row.get("delta_vs_fixed_construct_map"))
        if total_delta is not None and fixed_delta is not None and total_delta < 0.0 and fixed_delta < 0.0:
            improved_vs_both += 1
    ...
    if not identity_summary.empty:
        row = identity_summary[
            ...
            & (identity_summary["model"] == "M2b_total_anchored_residual_measurement")
        ]
        if not row.empty:
    ...
    identity_gate_passed = pred_identity is not None and pred_identity <= CURRENT_MV08_M2_PREDICTION_IDENTITY_BA + 1e-12
    if improved_vs_both < 2:
        status = "blocked_not_better_than_total_and_fixed_floors"
    elif not identity_gate_passed:
        status = "blocked_prediction_identity_increased_vs_mv08"
    else:
        status = "pass_total_anchored_residual_measurement_candidate"

    return {
        "pass_rule_status": status,
        "pass_rule_met": status.startswith("pass_"),
        "pooled_active_slices": int(len(active)),
        "pooled_m2b_improved_vs_total_score_floor_slices": int(improved_vs_total),
        "pooled_m2b_improved_vs_fixed_map_slices": int(improved_vs_fixed),
        "pooled_m2b_improved_vs_both_floor_slices": int(improved_vs_both),
        "pooled_m2b_worst_delta_vs_total_score_floor": safe_float(worst_vs_total),
        "pooled_m2b_worst_delta_vs_fixed_construct_map": safe_float(worst_vs_fixed),
        "feature_identity_ba": feature_identity,
        "prediction_identity_ba_m2b": pred_identity,
        "current_mv08_m2_prediction_identity_ba_gate": CURRENT_MV08_M2_PREDICTION_IDENTITY_BA,
        "prediction_identity_gate_passed": identity_gate_passed,
        "short_read": (
...
def artifact_hygiene(out_dir: Path) -> dict[str, Any]:
    ...
    return {
        "audit_id": "P5_MV08b_total_anchored_residual_measurement_hygiene",
...
def write_report(out_dir: Path, run_summary: dict[str, Any], comparison: pd.DataFrame, identity: pd.DataFrame, split_audit: pd.DataFrame) -> None:
    ...
    pooled = comparison[
        (comparison["protocol"] == "pooled_partial_invariance")
        & (comparison["model"].isin(["B1_total_score_floor", "B2_fixed_construct_map", "M2b_total_anchored_residual_measurement"]))
    ].copy()
    lines = [
...
def main() -> None:
    ...
    predictions, split_audit, residual_audit, threshold_summary = run_experiments(subjects, feature_cols, item_specs, args.split_path)
    ...
    residual_audit.to_csv(out_dir / "residual_model_audit.csv", index=False)
    threshold_summary.to_csv(out_dir / "threshold_sparsity_summary.csv", index=False)

    run_summary = {
        "run_id": RUN_ID,
        "generated_at": generated_at,
        "status": "complete",
        "scope": "minimal_validation_total_anchored_residual_no_full_method",
        "input_contract": {
        ...
        "predeclared_pass_rule": {
            "minimum_pooled_active_slices_beating_total_and_fixed_floors": 2,
            "prediction_identity_ba_must_not_exceed": CURRENT_MV08_M2_PREDICTION_IDENTITY_BA,
        ...
        "output_policy": {
            ...
            "residual_predictions_written": True,
            "learned_thresholds_written": False,
