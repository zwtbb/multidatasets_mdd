...
- The imported local human workbook has 288 annotation rows over 144 unique
  candidate IDs, 30 completed candidates, and 20 double-annotated candidates.
- `agreement_summary.csv` is dataset-stratified and also includes an `ALL`
  diagnostic row. Overall evidence-presence kappa is `0.808`; CMDC
  evidence-presence kappa is `0.643`; PDCH evidence-presence kappa is `1.000`;
  E-DAIC has only 2 double pairs with degenerate marginals, so kappa is
  undefined for E-DAIC in this first pass.
- The gate no longer blocks first-round aggregate MV06 evidence reporting, but
  stronger cross-dataset RQ4 claims should expand the E-DAIC double-annotation
  slice or add uncertainty/alpha-style agreement analysis.
...
stronger manuscript claim, add more E-DAIC double annotations or report
uncertainty/alpha-style agreement alongside the dataset-stratified kappa table.
