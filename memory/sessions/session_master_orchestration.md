...
- Phase 5 `P5_MV09 conditional_dataset_identity_audit` completed in the main
  ...
  severity-residualized BA). The next RQ1 work package is a classical
  psychometric invariance baseline, not another small BGE head iteration.
...
- Diagnostic paper claim/evidence tables now exist at
  ...
  allowed/blocked claim language, seven key numeric findings, and literature
...
7. Minimal validation:
   ...
   claim.
	   ...
	   and shows conditional BGE identity remains high. Freeze MV08/MV08b as
...
evidence and use `docs/diagnostic_measurement_audit_paper_outline.md` plus
`analysis/diagnostic_measurement_audit_paper/` as the next writing frame. The
Data Governance and Label Contracts scaffold is now ready; the next useful
writing task is the Baselines and Failure-Mode Diagnostics section. Optionally
expand E-DAIC MV06 double annotation to stabilize per-dataset agreement before
stronger RQ4 wording. Keep row-level predictions, real manifests, real
integrity/split maps, latent scores, learned parameters, and learned embeddings
