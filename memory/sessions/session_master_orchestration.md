...
next useful writing task is the Data Governance and Label Contracts section;
optionally expand E-DAIC MV06 double annotation to stabilize per-dataset
agreement. Keep row-level predictions, real manifests, real integrity/split
maps, latent scores, learned parameters, and learned embeddings local-only, and
