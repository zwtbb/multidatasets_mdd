...
- Diagnostic paper claim/evidence tables now exist at
  ...
  allowed/blocked claim language, ten key numeric findings, and literature
...
7. Minimal validation:
   ...
   claim.
	   ...
	   `P5_MV12` predeclares the two-stage latent-target design. Freeze MV08/MV08b as
	   ...
	   The next active experiment should be the `P5_MV12` two-stage
	   latent-target run, with theta scores, fitted parameters, row predictions,
	   transformed features, projection directions, and model artifacts kept
	   local-only.
...
confirmed the MV10 anchor map with a BIC caveat. MV12 now predeclares the
two-stage latent-target contract. The next research task is to implement and
run `scripts/phase5_run_mv12_two_stage_latent_target.py` under that contract:
generate local-only `Y -> theta` targets, compare `X -> theta` against direct
and floor baselines, and audit conditional identity/external transfer. The next
useful writing task is the Baselines and Failure-Mode Diagnostics section.
