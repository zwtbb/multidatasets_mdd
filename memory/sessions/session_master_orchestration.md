...
- Diagnostic paper claim/evidence tables now exist at
  ...
  allowed/blocked claim language, nine key numeric findings, and literature
...
7. Minimal validation:
   ...
   claim.
	   ...
	   anchors, and `P5_MV11` formally confirms that map with a BIC caveat. Freeze MV08/MV08b as
	   ...
	   The next active experiment should be `P5_MV12` two-stage latent-target
	   design, with factor scores and fitted parameters kept local-only.
...
confirmed the MV10 anchor map with a BIC caveat. The next research task is
`P5_MV12` two-stage latent-target design: define how local-only `Y -> theta`
targets will be fit, how `X -> theta` predictors will be compared with direct
and floor baselines, and how conditional identity/external transfer will be
audited. The next useful writing task is the Baselines and Failure-Mode
