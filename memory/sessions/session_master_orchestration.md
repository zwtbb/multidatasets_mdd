...
- Phase 5 `P5_MV12 two_stage_latent_target` completed in the main checkout.
  ...
  worse than direct itemwise Ridge and external theta transfer fails.
...
- Phase 5 `P5_MV14 measurement_uncertainty_bootstrap` completed in the main
  ...
  core effective R is `185`, DIF effective R is `100`, artifact hygiene passed,
  stable anchors are `C01/C04/C05/C07`, threshold DIF remains concentrated on
  `C02/C06`, and AIC/BIC model selection remains split
  (`configural`/`scalar`). Treat it as cautious PHQ partial-invariance
  uncertainty evidence only; full method remains blocked.
...
- Diagnostic paper claim/evidence tables now exist at
  ...
  allowed/blocked claim language, fourteen key numeric findings, and literature
...
7. Minimal validation:
   ...
   claim.
	   ...
	   because observed-scale safety and external theta transfer fail. Its
	   ...
	   convergence caveat. `P5_MV14` now completes the aggregate-only
	   measurement-uncertainty bootstrap run and supports cautious PHQ
	   partial-invariance wording.
	   ...
	   is now refreshed. The next active task is predeclaring MV15
	   latent-conditioned dataset identity, followed by MV16 cross-dataset theta
	   calibration / few-shot scale
...
bootstrap uncertainty is complete and supports cautious PHQ partial-invariance
wording; next, predeclare MV15 latent-conditioned identity, then MV16 few-shot
theta calibration.
