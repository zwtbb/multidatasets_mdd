...
- Diagnostic paper claim/evidence tables now exist at
  ...
  allowed/blocked claim language, eleven key numeric findings, and literature
...
7. Minimal validation:
   ...
   claim.
	   ...
	   because observed-scale safety and external theta transfer fail. Freeze
	   MV08/MV08b as negative RQ1 diagnostic evidence under the current
	   ...
	   The next active research decision should be whether to add aggregate-only
	   MV12 error/Pareto analysis or freeze the current latent-target line, with
	   theta scores, fitted parameters, row predictions, transformed features,
	   projection directions, and model artifacts kept local-only.
...
reconstruction and external theta transfer do not pass. The next research task
is to decide whether to add aggregate-only MV12 error/Pareto analysis or freeze
the current latent-target line. The next useful writing task is the Baselines
and Failure-Mode Diagnostics section.
