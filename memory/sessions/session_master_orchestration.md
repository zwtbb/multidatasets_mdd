...
- Diagnostic paper claim/evidence tables now exist at
  ...
  allowed/blocked claim language, seventeen key numeric findings, and literature
...
7. Minimal validation:
   ...
   claim.
	   ...
	   conditioning. `P5_MV16` is complete and negative/bounded:
	   the predeclared DIF-guided few-shot calibration ladder passes split,
	   ladder, anchor-safety, direct-baseline, output-identity, and hygiene gates,
	   but fails the both-direction small-k mechanism gate
	   (`blocked_no_dif_guided_small_k_gain`). Best supported row is
	   D1/M16d at k=10 with best L4 small-k delta theta MAE versus L0 `-0.227`,
	   while L4 small-k output identity BA remains `0.984`.
