...
| --- | --- | --- | --- |
| RQ1 | Do depression models measure the same construct across datasets and scales? | Direct shared symptom mapping is too strong under current Phase 5 evidence. Partial-invariance and total-anchored residual measurement are useful diagnostic frames, but current frozen-feature shallow implementations do not establish a transferable shared-symptom representation. MV10/MV11/MV19 are the primary bounded label-only PHQ common-structure and dataset-group threshold-shift evidence, with C02/C06 downgraded by observed-N finite-sample simulation. MV13/MV14 remain useful mirt qualitative screens, but a code-level parameterization audit shows their current calls fix the CMDC latent mean/variance, so they cannot be used as final anchor-linked DIF evidence until corrected or explicitly limited. MV12/MV15/MV16 form a bounded/negative latent-target and calibration consequence chain rather than a method pass. | E-DAIC: PHQ-8; CMDC: PHQ-9 plus limited HAMD fields; PDCH: HAMD-17. MPDD and EATD stay stress/context datasets until item-level contracts improve. |
...
evidence is `MV10/MV11/MV19` primary plus `MV13/MV14` limited mirt
qualitative screens; paper-support evidence is
