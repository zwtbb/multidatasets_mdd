...
| --- | --- | --- | --- |
| RQ1 | Do depression models measure the same construct across datasets and scales? | Direct shared symptom mapping is too strong under current Phase 5 evidence. Partial-invariance and total-anchored residual measurement are useful diagnostic frames, but current frozen-feature shallow implementations do not establish a transferable shared-symptom representation. MV10/MV11/MV13/MV14/MV19 support bounded label-only PHQ common-structure and dataset-group threshold-shift evidence, with C02/C06 downgraded by observed-N finite-sample simulation. MV12/MV15/MV16 form a bounded/negative latent-target and calibration consequence chain rather than a method pass. | E-DAIC: PHQ-8; CMDC: PHQ-9 plus limited HAMD fields; PDCH: HAMD-17. MPDD and EATD stay stress/context datasets until item-level contracts improve. |
...
evidence is reported with MV14 convergence uncertainty and MV19 finite-sample
downgrade, protocol and content controls remain central for RQ2, moderator-aware
