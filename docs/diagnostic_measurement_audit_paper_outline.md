...
9. Two-stage latent-target prediction is informative but still blocked.
   ...
   utility and conditional identity but fails observed-scale safety and
   external theta transfer, so it becomes measurement-shift evidence rather
   than a positive method result.
...
6. Psychometric Measurement Baselines
   ...
   - MV12 two-stage latent-target design and blocked run.
   - Label-only scale linking before multimodal prediction.
   - Next gate: decide whether an aggregate MV12 error/Pareto analysis reveals
     a predeclared mechanism change, or freeze this line as diagnostic evidence.
...
- `key_numeric_findings.csv`: eleven manuscript-ready findings for the full gate,
  RQ1 measurement sequence, MV10 psychometric baseline, MV11 formal
  confirmation, MV12 design, MV12 run, MV09 conditional identity, PDCH HAMD,
  MODMA task control, EATD stress, and MV06 evidence localization.
...
9. Decide whether to add aggregate-only MV12 error/Pareto analysis or freeze
   MV12 as the current bounded diagnostic result.
10. Draft the Baselines and Failure-Mode Diagnostics section from Phase 2 and
   Phase 3 aggregate summaries.
11. Expand E-DAIC MV06 double annotation if a stronger RQ4 claim is desired.
12. Prepare result tables from existing aggregate summaries only; do not export
