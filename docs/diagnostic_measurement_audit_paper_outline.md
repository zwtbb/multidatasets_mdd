...
iterations. MV14 now predeclares the next measurement-uncertainty layer:
group-wise subject bootstrap over the same PHQ item-response boundary, with
aggregate-only stability summaries for anchor support, loading/threshold DIF,
model selection, convergence, item fit, and SE/CI availability.
...
- MV14 provides a predeclared measurement-uncertainty/bootstrap design. It
  fixes the smoke/core/DIF/optional bootstrap tiers, local-only boundaries,
  stability metrics, and pass/downgrade rules before execution. It is not yet a
  bootstrap result and does not authorize full method work.
...
8. Formal and external label-only IRT confirmation preserve the partial-anchor story.
   ...
   limitation rather than hiding it. MV14 is now predeclared to test whether
   that single-fit anchor/DIF pattern is stable under group-wise subject
   bootstrap before stronger item-level wording.
...
6. Psychometric Measurement Baselines
   ...
   - MV14 measurement-uncertainty/bootstrap design and future stability run.
...
13. Implement and run the predeclared MV14 bootstrap, exporting aggregate
