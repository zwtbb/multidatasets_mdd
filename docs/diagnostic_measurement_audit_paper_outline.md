...
DIF-guided few-shot measurement calibration. MV16 is now predeclared for that
last step; the next work is implementing its runner, not changing the design
after seeing calibration results.
...
17. Next: implement and run the MV16 calibration runner with aggregate curves
   only and local-only theta/calibration/row artifacts.
18. Add MV06 agreement uncertainty analysis and resolve the remaining
   incomplete local candidate if stronger RQ4 wording is desired.
19. Prepare result tables from existing aggregate summaries only; do not export
