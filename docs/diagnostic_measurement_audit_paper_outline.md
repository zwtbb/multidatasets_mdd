...
13. Predeclare MV15 latent-conditioned dataset identity: compare `D|Z`,
   ...
   identity outputs.
14. Predeclare MV16 cross-dataset theta calibration / few-shot scale linking:
   evaluate affine, monotonic, or anchor-based calibration with small target
   calibration sets before any new full method.
15. Expand E-DAIC MV06 double annotation if a stronger RQ4 claim is desired.
16. Prepare result tables from existing aggregate summaries only; do not export
