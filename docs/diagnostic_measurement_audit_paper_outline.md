...
records an AIC/BIC caveat. MV12 now predeclares the two-stage latent-target
...
and external transfer. Together, MV10/MV11/MV12 move the project from a generic
benchmark audit toward a target-measurement-shift paper; the next gate is the
actual MV12 run before any full method.
...
9. Two-stage latent-target prediction is the next predeclared method test.
   ...
   E-DAIC/CMDC external transfer. The design is ready, but it is not yet a
...
6. Psychometric Measurement Baselines
   ...
   - MV12 two-stage latent-target design.
   - Label-only scale linking before multimodal prediction.
   - Next gate: implement and run the predeclared plan that fits measurement
     model `Y -> theta`, trains multimodal predictor `X -> theta`, then maps
     `theta -> Y^(d)`.
...
- `key_numeric_findings.csv`: ten manuscript-ready findings for the full gate,
  RQ1 measurement sequence, MV10 psychometric baseline, MV11 formal
  confirmation, MV12 design, MV09 conditional identity, PDCH HAMD, MODMA task
...
8. Implement and run the MV12 two-stage latent-target experiment.
9. Draft the Baselines and Failure-Mode Diagnostics section from Phase 2 and
   Phase 3 aggregate summaries.
10. Expand E-DAIC MV06 double annotation if a stronger RQ4 claim is desired.
11. Prepare result tables from existing aggregate summaries only; do not export
